...
class RegisterUser{
	...
	private function checkValidationOfValues(){
		if(strlen($this->login)<6){
			...
			return true;
		}elseif(strlen($this->raw_password)<6 || preg_match("/\W/",$this->raw_password) || (!preg_match('/[A-zА-я]+/', $this->raw_password)) || (!preg_match('/[0-9]+/', $this->raw_password))){
			$this->error = "Пароль должен быть минимум 6 символов,состоять из цифр и букв,а также недопустимы спецсимволы";
			return true;
			...
			$this->error = "Длина имени должна быть минимум 2 символов";
			return true;
		}elseif(preg_match("/\s/",$this->login) || preg_match("/\s/",$this->raw_password) || preg_match("/\s/",$this->name)){
			$this->error = "Пробелы запрещины в полях ввода";
			return true;
		}else{
			return false;
		}
